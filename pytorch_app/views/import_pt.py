...
def import_model(request):
    ...
    while(operations):
        ...
        if node_name == 'MmBackward' or node_name == 'AddmmBackward':
            d[node]['params'][] = (str(list(node.next_functions[2][0].next_functions[0][0].variable.size())).replace('L',''))[1:-1]
            if node_name == 'AddmmBackward':
                d[node]['params'][] = (str(list(node.next_functions[0][0].next_functions[0][0].variable.size())).replace('L',''))[1:-1]
        """
